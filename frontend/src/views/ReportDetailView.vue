              ...
              >
                <div class="flex items-center space-x-3">
                  <div
                    ...
                    :class="getViolationColor(violation.severity)"
                  ></div>
...
const report = ref({
  ...
  summary: {
    ...
    totalViolations: 24,
    description:
      "Overall compliance has improved by 5% compared to last month. HOS violations remain the primary concern, accounting for 60% of all violations.",
  },
  violations: [
...
const downloadReport = () => {
  try {
    ...
    link.href = url;
    link.download = `${report.value.title.replace(/\s+/g, "_")}_Detailed_${new Date().toISOString().split("T")[0]}.csv`;
...
const generateDetailedReportContent = () => {
  ...
  // Header
  lines.push("AUDIT REPORT DETAILS");
  lines.push("");
...
const exportToPDF = () => {
  try {
    // Generate detailed report content for PDF export
    const reportContent = generateDetailedReportContent();
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `${report.value.title.replace(/\s+/g, "_")}_PDF_Export_${new Date().toISOString().split("T")[0]}.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);

    showSuccess("Export Complete", "Report has been exported successfully.");
  } catch (error) {
